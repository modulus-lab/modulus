...
export async function loadMockRouters(): Promise<Router> {
  try {
    ...
    for (const dir of directories) {
      ...
      const router = await getMockRouter(dir);
      if (!router) continue;
...
async function getMockRouter(dir: fs.Dirent): Promise<Router | null> {
  const serviceName = dir.name;
  ...
  console.log(`  → Skipping ${serviceName} (no index.json file found)`);
  return null;
}
...
async function getJSRouter(indexPath: string, serviceName: string) {
  try {
    ...
    const module = await import(`${indexPath}/index.js`);
    if (!module.default || typeof module.default !== 'function') {
      console.log(`  ✗ Module invalid (no default export or default is not a function)`);
      return null;
    }
...
async function getJSONRouter(indexPath: string, serviceName: string) {
  ...
  let router;
  if (json.useCustomRouter) {
    console.log(`  → Config: Using js router`);
    router = await getJSRouter(indexPath, serviceName);
    router.prototype.responses = json.responses;
  } else {
    router = express.Router();

    const responses = fs.readdirSync(indexPath)
      .filter(it => it.endsWith('.json') && !it.startsWith('index'));
    console.log(`  → Found ${responses.length} response(s): ${responses.join(', ')}`);

    router.prototype.responses = responses.map((it: any) => {
    ...
    if (fs.existsSync(path.join(indexPath, `proxy.js`)) || fs.existsSync(path.join(indexPath, `proxy.ts`))) {
  ...
  if (!json.useCustomRouter) {
    ...
    const method = json.method as "get" | "post" | "put" | "delete" | "patch" | "options";
    console.log(`  → Registering route: ${method.toUpperCase()} ${json.path}`);
    router[method](json.path, async (req, res) => {
      const mappings = await getMappingsFromReq(req, serviceConfigs[serviceName].uniqueKey) as any;
      const expectedResponse = mappings && mappings.responses
      ...
      const responseData = fs.readFileSync(path.join(indexPath, `${expectedResponse}.json`));
      const responseJson = JSON.parse(responseData.toString());
...
async function getMappingsFromReq(req: Request, uniqueKey?: UniqueKey) {
  ...
  console.log(`  → Looking for unique key: ${uniqueKey.type} = ${uniqueKeyValue} (from ${uniqueKey.target}.${uniqueKey.modifier})`);

  if (!uniqueKeyValue) {
    console.log(`  → Unique key value not found in request, using default response`);
    return null;
  }
  ...
  console.log(`  → ${filtered ? "Found a" : "Not found any"} matching response mapping`);
  return filtered;
}
