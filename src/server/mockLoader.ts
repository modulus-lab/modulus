...
async function getMockRouter(dir: fs.Dirent): Promise<Router | null> {
  ...
  if (fs.existsSync(path.join(indexPath, "index.ts")) || fs.existsSync(path.join(indexPath, "index.js"))) {
...
async function getJSONRouter(indexPath: string, serviceName: string) {
  ...
  console.log(`  → Found ${responses.length} response file(s): ${responses.join(', ')}`);
